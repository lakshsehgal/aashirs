...
/**
 * MIS Dashboard built from the uploaded PDF (Sept 1–24, 2025 slice).
 * Visuals: KPI cards, trend charts, and a detailed day-by-day table for Total, Meta, and Google.
 * Styling: TailwindCSS only (keeps the preview portable). No external UI libs required.
 */

// ---- Raw daily data parsed from the PDF ----
// Note: All currency values are in INR (₹). Dates in DD/MM/YYYY.
const daily = [
...
// Overall period KPIs (current vs previous) from the PDF
const periodKPIs = {
...
function Dashboard() {
  ...
  const trend = useMemo(() => {
    return daily.map((d) => ({
      date: d.date.slice(0, 5), // show DD/MM
